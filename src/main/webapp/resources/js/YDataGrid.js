var YDataGrid = function(config) {
	...
	var Handler = {
		...
		edit : function(callback) {
			...
			if (Utils.checkSelectOne(record)) {
				...
				DeepAi.getById(Action.getId, data, function(result) {
			...
			if (Utils.checkSelect(records)) {
				$.messager.confirm('Confirm',
						'Are you sure you want to delete record?', function(r) {
							if (r) {
								var idKey = dataGrid.idField || 'id'; // 主键名称
								var data = $("input[name='" + idKey + "']",
										Form.list).fieldSerialize(); // 序列化字段
								DeepAi.deleteForm(Action.remove, data,
										function(result) {
											Events.refresh();
											// 回调函数
											if (jQuery.isFunction(callback)) {
		...
		save : function(callback) {
			if (Form.edit.form('validate')) {
				...
				DeepAi.saveForm(Form.edit, function(data) {
		...
		close : function(callback) {
			$.messager.confirm('Confirm',
					'Are you sure you want close Window?', function(r) {
						if (r) {
							Win.edit.dialog('close');
							// 回调函数
							if (jQuery.isFunction(callback)) {
	...
	var Utils = {
		...
		checkSelectOne : function(rows) {// 检查grid是否只勾选了一行,是返回 true,否返回true
			...
			DeepAi.alert('警告', '只能选择一行记录.', 'warning');
			return false;
		}
	}
	...
	var initTbar = function() {
		...
		// 查询页面授权的btnType
		DeepAi.ajaxJson(_url, data, function(data) {
			if (data.success) {
				if (data.allType) {
					Grid.datagrid({
						'toolbar' : tbars
