...
var DeepAi={
	...
	checkLogin:function(data){//检查是否登录超时
		if(data.logoutFlag){
			DeepAi.closeProgress();
			DeepAi.alert('提示',"登录超时,点击确定重新登录.",'error',DeepAi.toLogin);
			return false;
		}
		return true;
	},
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!DeepAi.checkLogin(data)){
					 		...
					 		DeepAi.alert('提示', data.msg || "请求出现异常,请联系管理员",'error');
					 	}
			 		}catch(e){
			 			alert(e);
			 			DeepAi.alert('提示',"请求出现异常,请联系管理员1",'error');
			 		}
			 	},
			 	complete:function(){
	...
	submitForm:function(form,callback,dataType){
			var option =
			{
			 	...
			 	error:function(response, textStatus, errorThrown){
			 		try{
				 		...
				 		if(!DeepAi.checkLogin(data)){
					 		...
					 		DeepAi.alert('提示1', data.msg || "请求出现异常,请联系管理员",'error');
					 	}
			 		}catch(e){
			 			alert(e);
			 			DeepAi.alert('提示2',"请求出现异常,请联系管理员1",'error');
			 		}
			 	},
			 	complete:function(){
			 ...
			 DeepAi.ajaxSubmit(form,option);
	},
	saveForm:function(form,callback){
		if(form.form('validate')){
			DeepAi.progress('Please waiting','Save ing...');
			//ajax提交form
			DeepAi.submitForm(form,function(data){
				DeepAi.closeProgress();
			 	if(data.success){
			 		if(callback){
				       	callback(data);
				    }else{
			       		DeepAi.alert('提示','保存成功.','info');
			        } 
		        }else{
		       	   DeepAi.alert('提示',data.msg,'error');  
		        }
			});
		 }
	},
	...
	getById:function(url,option,callback){
		DeepAi.progress();
		DeepAi.ajaxJson(url,option,function(data){
			DeepAi.closeProgress();
			if(data.success){
				if(callback){
			       	callback(data);
	...
	deleteForm:function(url,option,callback){
		...
		DeepAi.ajaxJson(url,option,function(data){
				...
				if(data.success){
					...
					DeepAi.alert('提示',result.msg,'error');  
				}
		});
	}
}
